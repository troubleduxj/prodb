...
// MQTTProtocol implements MQTT protocol
...
func (m *MQTTProtocol) SetConnected(connected bool) {
	m.connected = connected
}

// Connect establishes connection to MQTT broker
func (m *MQTTProtocol) Connect(configMap map[string]interface{}) error {
...
func (m *MQTTProtocol) parseConfig(configMap map[string]interface{}) (*MQTTConfig, error) {
	config := &MQTTConfig{
...
func (m *MQTTProtocol) parseTopics(configMap map[string]interface{}) error {
...
func (m *MQTTProtocol) createClientOptions() *mqtt.ClientOptions {
...
func (m *MQTTProtocol) subscribeToTopics() error {
...
func (m *MQTTProtocol) createMessageHandler(topicConfig *TopicConfig) mqtt.MessageHandler {
...
func (m *MQTTProtocol) parseMessage(msg mqtt.Message, topicConfig *TopicConfig) ([]DataValue, error) {
	now := time.Now()
	payload := msg.Payload()
...
func (m *MQTTProtocol) parseJSONMessage(payload []byte, topic string, topicConfig *TopicConfig, timestamp time.Time) ([]DataValue, error) {
...
func (m *MQTTProtocol) parseRawMessage(payload []byte, topic string, topicConfig *TopicConfig, timestamp time.Time) ([]DataValue, error) {
	dataValue := DataValue{
		DeviceID:  m.getDeviceID(topicConfig, topic),
		PointName: m.getPointName(topicConfig, topic),
		Timestamp: timestamp,
...
func (m *MQTTProtocol) parseCSVMessage(payload []byte, topic string, topicConfig *TopicConfig, timestamp time.Time) ([]DataValue, error) {
...
// extractJSONPath extracts value from JSON using simple JSONPath-like syntax
func (m *MQTTProtocol) extractJSONPath(data interface{}, path string) (interface{}, error) {
	// Simple JSONPath implementation - supports basic dot notation
	// e.g., "data.temperature", "sensors[0].value"
...
// inferDataType infers data type from value
func (m *MQTTProtocol) inferDataType(value interface{}) string {
	switch value.(type) {
	case bool:
		return "bool"
...
// getDeviceID returns device ID for the data point
func (m *MQTTProtocol) getDeviceID(topicConfig *TopicConfig, topic string) string {
...
// getPointName returns point name for the data point
func (m *MQTTProtocol) getPointName(topicConfig *TopicConfig, topic string) string {
	if topicConfig.PointName != "" {
		return topicConfig.PointName
	}

	// Generate point name from topic
	return m.sanitizeTopicForID(topic)
}

// sanitizeTopicForID sanitizes topic name for use as ID
func (m *MQTTProtocol) sanitizeTopicForID(topic string) string {
...
// createTags creates tags for the data point
func (m *MQTTProtocol) createTags(topicConfig *TopicConfig, topic string) map[string]string {
...
func (m *MQTTProtocol) createTLSConfig() (*tls.Config, error) {
...
func (m *MQTTProtocol) defaultMessageHandler(client mqtt.Client, msg mqtt.Message) {
...
func (m *MQTTProtocol) connectionLostHandler(client mqtt.Client, err error) {
...
// onConnectHandler handles successful connection events
func (m *MQTTProtocol) onConnectHandler(client mqtt.Client) {
...
// reconnectingHandler handles reconnection attempts
func (m *MQTTProtocol) reconnectingHandler(client mqtt.Client, opts *mqtt.ClientOptions) {
	m.logger.Info("Attempting to reconnect to MQTT broker")
}

// GetConnectionStatus returns detailed connection status
func (m *MQTTProtocol) GetConnectionStatus() map[string]interface{} {
	status := make(map[string]interface{})

	status["connected"] = m.IsConnected()
	status["broker"] = fmt.Sprintf("%s:%d", m.config.Broker, m.config.Port)
	status["client_id"] = m.config.ClientID
	status["subscribed_topics"] = len(m.topics)

	if m.client != nil {
		status["client_connected"] = m.client.IsConnected()
	}

	// Get buffer status
	m.bufferMutex.RLock()
	bufferCounts := make(map[string]int)
	totalBuffered := 0
	for topic, buffer := range m.dataBuffer {
		count := len(buffer)
		bufferCounts[topic] = count
		totalBuffered += count
	}
	m.bufferMutex.RUnlock()

	status["buffered_messages"] = totalBuffered
	status["buffer_by_topic"] = bufferCounts

	return status
}

// GetTopicList returns list of subscribed topics
func (m *MQTTProtocol) GetTopicList() []string {
...
// ClearBuffer clears the message buffer for all topics
func (m *MQTTProtocol) ClearBuffer() {
	m.bufferMutex.Lock()
	defer m.bufferMutex.Unlock()

	for topic := range m.dataBuffer {
		m.dataBuffer[topic] = nil
	}

	m.logger.Info("MQTT message buffer cleared")
}

// RegisterMQTTProtocol registers MQTT protocol with the protocol manager
func RegisterMQTTProtocol(registerFunc func(string, interface{}) error, logger *logger.Logger) error {
	return registerFunc("mqtt", NewMQTTProtocol(logger))
}
